...
```{r}
p<-ggplot(mtcars, aes(x = wt, y = mpg))+ geom_point()+ labs(title = "Motor Trend Car Road Tests",
...
```{r}
ggplot(mtcars, aes(x = wt, y = mpg, color=vs))+ geom_point()+ labs(title = "Motor Trend Car Road Tests",
       subtitle = "Relación entre el peso de un vehículo y su consumo",
       x = "Weight (1000 lbs)",
       y = "Miles/(US) gallon",
       caption = "The data was extracted from the 1974 Motor Trend US magazine.")
...
```{r}
grafico3 <- ggplot(ds, aes(x=MinTemp, y=MaxTemp, colour=RainTomorrow)) + geom_point() + ggtitle("Hola")
grafico3
```
...
```{r}
ggplot(ds, aes(x=Temp9am, y=Pressure9am, colour=RainToday)) + geom_point() + labs(title="Hola", subtitle = "No se que pner", x="hi", y="hey")
```
...
```{r}
iris.grafico2 <- ggplot(iris.tidy, aes(x=Species, y=Value, shape= Part, colour=Part)) + geom_point()
iris.grafico2
```

```{r}
iris.grafico3 <- ggplot(iris.tidy, aes(x=Species, y=Value, shape= Part, colour=Part)) + geom_point() + facet_grid(~Measure)
iris.grafico3
```

```{r}
iris.grafico3 <- ggplot(data=iris.tidy, aes(x=Species, y=Value, shape=Part, color=Part)) + geom_point() + facet_grid(Measure~.)
iris.grafico3
```

```{r}
iris.grafico3 <- ggplot(data=iris.tidy, aes(x=Species, y=Value, shape=Part, color=Part)) + geom_jitter() + facet_grid(Measure~.)
iris.grafico3
```
...
iris.grafico2 <- ggplot(data=iris.tidy, aes(x=Species, y=Value, shape=Part, color=Part)) + geom_point()
iris.grafico2
...
iris.grafico4 <- ggplot(data=iris.tidy, aes(x=Species, y=Value, shape=Part, color=Part)) + geom_jitter() + facet_grid(Measure~.)
iris.grafico4
...
m <- ggplot(DatosEjer4, aes(x=Fecha, y=Ventas, colour=Fabricante)) + geom_point() + geom_line()
n <- ggplot(DatosEjer4, aes(x=Fecha, y=Ventas, colour=Fabricante)) + geom_point() + geom_line(aes(linetype = Fabricante))
m
n
...
```{r}
ggplot(Experimento, aes(x=Dosis, y=Altura, linetype = Complemento, shape=Complemento)) + geom_point()


ggplot(Experimento, aes(x = Dosis, y = Altura, linetype = Complemento, shape = Complemento)) +
  geom_point() +
  geom_line() +
  geom_errorbar(aes(ymin = Altura - Error_Altura, ymax = Altura + Error_Altura), width = 0.2)


library(ggplot2)

ggplot(Experimento, aes(x = Dosis, y = Altura, colour = Complemento)) +
...
ggplot(ds, aes(x = factor(Year), y = MinTemp)) +
  geom_boxplot() +
  labs(
    title = "Distribución de temperaturas mínimas por año",
    x = "Año",
